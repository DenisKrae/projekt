@push('css')
<style>
    .severity-0 { background-color: green !important; }
    .severity-1 { background-color: yellow !important; }
    .severity-2 { background-color: red !important; }
</style>
@endpush
    ...
    <table class="table" id="wartung-seite">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="severity-2">{{ $module_severity_high }} </td>
                <td class="severity-1">{{ $module_severity_low }} </td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
<script>
    $(document).ready(function () {
        ...
        $('#auftrag-filter').on('change', function() {            
            ...
            var columnToSort = "0"; // The column index to sort (change this to the appropriate column index)
            
            // Clear any existing sorting
            table.order([]).draw();

            // Use DataTables API to change the sorting order
            table.order([[columnToSort, selectedValue]]).draw();
        });
